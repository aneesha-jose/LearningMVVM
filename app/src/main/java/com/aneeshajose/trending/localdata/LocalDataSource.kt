 ...
 */
@ApplicationScope
class LocalDataSource @Inject constructor(@ApplicationContext context: Context,val coroutineContextProvider: CoroutineContextProvider) {

    private val appDatabase = AppDatabase.invoke(context)

    suspend fun getValidRepos(): List<Repo> = withContext(coroutineContextProvider.IO) {
    ...
    suspend fun refreshRepositoriesData(repos: List<Repo>) = withContext(coroutineContextProvider.IO) {
